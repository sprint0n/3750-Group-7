...
recordRoutes.route("/getTransfers").get(async (req, res) => {
  ...
  try{
    ...
    const results = await transferCollection.find(myquery).sort({transferDate: -1}).toArray();
    res.json(results);
  }catch (err){
    console.error(err);
...
recordRoutes.route("/register").post(async (req, res) => {
  try {
    ...
    return res.status(500).json({message: "An unexpected error occured during registration."})
  }
});
...
//allow user to Log IN
  try{
    ...
    return res.status(500).json({message: "An unexpected error occured during log in."})
    }
});
...
recordRoutes.route("/deposit").post(async (req, res) => {
  ...
  try{
    ...
    if(!amount || !account || !category){
      return res.status(400).json({message: "Missing Paramaters. Please fill all fields."});
    }
    ...
    return res.status(500).json({ message: "An unexpected error ocurred during deposit."})
  }
});
...
//allow user to withdraw money into chosen account
 if(!req.session.userId){
  ...
  try{
    ...
    if (userUpdate.modifiedCount === 0){
      return res.status(400).json({message: "Insufficient funds. Withdrawl cannot be completed."})
    }
    ...
    return res.status(500).json({ message: "An unexpected error occurred during withdrawal." });
  }
});
...
recordRoutes.route("/InternalTransfer").post(async (req, res) => {
  ...
  try{
    ...
    return res.status(500).json({ message: "An unexpected error occurred during transfer." });
  }
});
...
//allow user to transfer with another account, as long as you have given id and 1,2,3 for account to transfer with
  ...
  try{
    ...
    if (senderId.toString() === targetUserId) {
      return res.status(400).json({ message: "Cannot transfer to yourself. Use Internal Transfer." });
    }
    const transferAmount = parseFloat(amount);
    if (isNaN(transferAmount) || transferAmount <= 0) {
      return res.status(400).json({ message: "Invalid transfer amount." });
    ...
    let targetUser;
    try {
      targetUser = await userCollection.findOne({_id: new ObjectId(targetUserId)})
    } catch (e){
...
//grab the specific history of one account transfers
  ...
  try{
    ...
    if (!isValidAccount(account)){
      return res.status(400).json({messgae: "Please enter a valid account name."});
    }

    const accountID = `${account}_${userId}`;

    const results = await transferCollection.find({
      $or: [
        {AccountTransferedFrom: accountID},
        {AccountTransferedTo: accountID}
      ]
    }).sort({TransferDate: -1}).toArray();
